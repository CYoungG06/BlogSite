...
date: "2025-01-04"
description: "首先对于这个 Seq2Seq 模型，其编码器解码器都基于RNN，将注意力添加在解码器时，它接受 t 时间步的RNN的隐藏层输出 h_t 和所有编码器的隐藏层向量 hs"
tags: ["attention"]
---
> 前情提要🤓：  
...
$$
Q = W_qX \in \mathbb{R}^{D_k \times N} \\K = W_kX \in \mathbb{R}^{D_k \times N} \\V = W_vX \in \mathbb{R}^{D_v \times N}
$$
...
$$
\begin{align*}h_n = attn((\mathbf{K,V}), q_n) &= \sum_{j=1}^{N} \alpha_{nj} \pmb{v_j} \\&= \sum_{j=1}^{N} softmax(score(k_j, q_n)) \pmb{v_n}\end{align*}
$$
