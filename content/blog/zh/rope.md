...
```python
import torchimport mathdef rotate_every_two(x):    x1 = x[..., ::2]    x2 = x[..., 1::2]    return torch.stack([-x2, x1], dim=-1).reshape_as(x)def apply_rotary_pos_emb(q, k, cos, sin):    # Apply RoPE to query and key    q_rotated = (q * cos) - (rotate_every_two(q) * sin)    k_rotated = (k * cos) - (rotate_every_two(k) * sin)    return q_rotated, k_rotatedclass RoPE:    def __init__(self, dim, max_position=10000):        inv_freq = 1.0 / (10000 ** (torch.arange(0, dim, 2).float() / dim))        self.inv_freq = inv_freq    def __call__(self, pos, x):        # pos: [batch_size, seq_len]        # x: [batch_size, seq_len, dim]        sinusoid_inp = torch.einsum("i,j->ij", pos, self.inv_freq)        sin = sinusoid_inp.sin()[None, :, :]        cos = sinusoid_inp.cos()[None, :, :]        return apply_rotary_pos_emb(x, x, cos, sin)# 示例使用batch_size, seq_len, dim = 2, 50, 64  # 示例尺寸x = torch.randn(batch_size, seq_len, dim)  # 示例输入positions = torch.arange(seq_len).unsqueeze(0).repeat(batch_size, 1)  # 位置索引rope = RoPE(dim)x_rotated, _ = rope(positions, x)  # 应用RoPE
```
